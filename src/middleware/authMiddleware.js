    ...
    try {
        ...
        if (!admin && email) {
            ...
            if (admin && !admin.firebaseUid) {
                console.log(
                    `Found admin by email, linking Firebase UID ${firebaseUid} to local admin ID ${admin.id}`
                );
                ...
                console.error(
                    `CRITICAL: Email ${email} in DB is linked to UID ${admin.firebaseUid}, but token has UID ${firebaseUid}.`
                );
        ...
        if (!admin) {
            console.warn(
                `Firebase user ${firebaseUid} (${email || "no email"}) authenticated successfully, but no corresponding Admin record found in local database.`
            );
            return res.status(403).json({
                error: "Forbidden: Your account is authenticated but not registered as an admin."
            });
        }

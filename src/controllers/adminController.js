...
export const createAdmin = async (req, res) => {
    try {
        ...
        if (existingByUsername) {
            return res
                .status(409)
                .json({ error: "Admin with this username already exists locally." });
        }
        ...
        if (!frontendUrl) {
            console.warn(
                "FRONTEND_ADMIN_URL environment variable not set. Cannot send sign-up invitation email."
            );
            return res.status(201).json({
                message:
                    "Admin record created locally. Sign-up invitation email NOT sent (FRONTEND_ADMIN_URL not configured).",
                admin: newAdmin
            });
        }
        ...
        try {
            ...
            console.error(
                "Failed to send sign-up invitation email, but admin record was created.",
                emailError
            );
            res.status(201).json({
    ...
    try {
        ...
        if (adminToDelete.firebaseUid) {
            try {
                ...
                console.log(
                    `Successfully deleted Firebase user: UID ${adminToDelete.firebaseUid}, Email: ${adminToDelete.email}`
                );
            } catch (firebaseError) {
                console.error(
                    `Failed to delete Firebase user ${adminToDelete.firebaseUid} (Email: ${adminToDelete.email}):`,
                    firebaseError.code,
                    firebaseError.message
                );
                if (firebaseError.code !== "auth/user-not-found") {
                    console.warn(
                        "Proceeding with local database deletion despite Firebase deletion error."
                    );
                }
            }
        } else {
            console.warn(
                `Admin ID ${id} (Email: ${adminToDelete.email}) has no associated Firebase UID. Skipping Firebase deletion step.`
            );
        }
        ...
        res.status(200).json({
            message: "Admin deleted successfully from local DB and Firebase (if applicable)."
        });
    } catch (err) {
        ...
        if (err.code === "P2025") {
            return res
                .status(404)
                .json({ error: "Admin not found during final delete operation." });
        }

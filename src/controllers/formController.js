...
export const handleCareerApplication = async (req, res) => {
    ...
    if (!firstName || !phoneNumber || !email || !areaOfInterest || referred === undefined) {
        return res.status(400).json({
            error: "Missing required fields (First Name, Phone, Email, Area of Interest, Referred status)."
        });
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    ...
    try {
        ...
        console.log(
            `Attempting to send notification email for application ID: ${newApplication.id}`
        );
        try {
            ...
            if (adminEmails.length > 0) {
                await sendCareerApplicationNotification({
                    ...
                    applicantName:
                        `${newApplication.firstName} ${newApplication.lastName || ""}`.trim(),
                    applicantEmail: newApplication.email,
                    areaOfInterest: newApplication.areaOfInterest,
                    resumeFile: resumeFile
                });
                console.log(
                    `Notification email queued for ${adminEmails.length} admins for application ID: ${newApplication.id}`
                );
            } else {
            ...
            console.error(
                `Failed to send career application notification email for application ID ${newApplication.id}:`,
                emailError
            );
        }
        ...
        if (error.code === "P2002" && error.meta?.target?.includes("email")) {
            return res
                .status(409)
                .json({ error: `An application with this email address already exists.` });
        }
        ...
        res.status(500).json({
            error: "Failed to submit application due to a server error.",
...
export const handleContactInquiry = async (req, res) => {
    ...
    if (!fullName || !phoneNumber || !email || !message) {
        return res
            .status(400)
            .json({ error: "Missing required fields (Full Name, Phone, Email, Message)." });
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        return res.status(400).json({ error: "Invalid email format." });
    }
    ...
    try {
        ...
        try {
            ...
            if (adminEmails.length > 0) {
                ...
                console.log(
                    `Notification email queued for ${adminEmails.length} admins for inquiry ID: ${newInquiry.id}`
                );
            } else {
                console.log("No admin emails found in database to notify.");
            }
            ...
            console.error(
                `Failed to send contact inquiry notification email for inquiry ID ${newInquiry.id}:`,
                emailError
            );
        }
        res.status(201).json({
        ...
        res.status(500).json({
            error: "Failed to submit inquiry due to a server error.",
            details: error.message
        });
    }
};

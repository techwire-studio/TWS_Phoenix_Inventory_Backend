...
async function importCSV(filePath, importMode = 'skip') {
      ...
      .on('data', (row) => {
        ...
        if (!productsMap.has(productId)) {
          productsMap.set(productId, {
            ...
            imageUrls: parseStringOrNull(row.imageUrls)?.split(',').map(url => url.trim()) || [],
            ...
            dimensions: parseStringOrNull(row.dimensions) ? JSON.parse(row.dimensions) : null,
            weight: parseStringOrNull(row.weight) ? JSON.parse(row.weight) : null,
            otherDetails: parseStringOrNull(row.otherDetails) ? JSON.parse(row.otherDetails) : null,
            variants: [],
          });
        }
  ...
  if (productsToUpdate.length > 0) {
    if (importMode === 'skip') {
      console.log(`Skipping ${productsToUpdate.length} existing products as per 'skip' mode.`);
